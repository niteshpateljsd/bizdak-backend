...
// Apply general limiter to all API routes EXCEPT /events
...
// Logging — 'combined' in production, 'dev' locally
...
app.post(
  '/api/admin/backfill-translations',
  authenticate,
  async (req, res, next) => {
    try {
      if (_backfillRunning) {
        return res.status(409).json({
          error:
            'Backfill already in progress — please wait for it to complete.',
        });
      }
...
// Status endpoint
app.get(
  '/api/admin/backfill-status/:jobId',
  authenticate,
  (req, res) => {
    ...
    if (_lastBackfillResult.error) {
      return res.json({
        status: 'error',
        error: _lastBackfillResult.error,
      });
    }
